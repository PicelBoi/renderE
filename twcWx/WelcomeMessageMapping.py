...
class WelcomeMessageMappingHandler(xmlUtil.LookupSubHandler):

    def __init__(self, container):
        self._elements = [('welcomeMessage', str, xmlUtil.REQUIRED)]
...
filePath = rg.newjoin(os.environ["RENDEREMEDIA"], '/mappings/welcomeMessage/')

class WelcomeMessageMapping(mapping.Map):
    ...
    def _load(self, data):
        ...
        try:
            map = xmlUtil.parseXML(path, WelcomeMessageMappingHandler)
            if map:
                return (map, path)
            else:
                return None
